...
file_array2=(
"${flavor_dir}config.js"
"${flavor_dir}dimens.scss"
"${flavor_dir}colors.scss"
)

file_target_dir1="../"
file_target_dir2="./out/"
...
files_check "${file_array2[*]}"

# dir_check "${file_target_dir1}"
# dir_check "${file_target_dir2}"
...
# 创建输出目录
dir_exist ${file_target_dir2}
if [ $? -eq 0 ]
then
   echo "目标文件夹不存在！开始创建！"
   mkdir -p ${file_target_dir2}
fi

# 删除目标目录中的相应文件
# files_del "${file_array2_target[*]}"
echo "删除目录下所有文件 ${file_target_dir2}/*"
rm  -rf ${file_target_dir2}/*

#将文件夹中的文件复制到相应的位置，同时界面输出复制情况
#cp ${file_array2[0]} ${file_target_dir2}
files_cp "${file_array2[*]}" "${file_target_dir2}"
